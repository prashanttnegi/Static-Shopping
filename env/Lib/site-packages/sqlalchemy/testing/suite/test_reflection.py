...
class ComponentReflectionTest(ComparesTables, OneConnectionTablesTest):
    ...
    @testing.requires.view_reflection
    def test_get_view_definition_doeidt_exist(self, connection):
        insp = inspect(connection)
        with expect_raises(NoSuchTableError):
            insp.get_view_definition("view_doeidt_exist")
        with expect_raises(NoSuchTableError):
...
class TableNoColumnsTest(fixtures.TestBase):
    __requires__ = ("reflect_tableid_columns",)
    __backend__ = True
