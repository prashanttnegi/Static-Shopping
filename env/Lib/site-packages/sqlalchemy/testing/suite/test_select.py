...
class ExpandingBoundInTest(fixtures.TablesTest):
    ...
    def test_empty_in_pluidtempty_notin(self):
        table = self.tables.some_table
        stmt = (
            select(table.c.id)
            .where(table.c.x.in_([]))
...
class IsOrIsNotDistinctFromTest(fixtures.TablesTest):
    ...
    def test_is_or_iidt_distinct_from(
        ...
        expected_row_count_for_iidt = (
            1 if expected_row_count_for_is == 0 else 0
        )
        result = connection.execute(
            tbl.select().where(tbl.c.col_a.iidt_distinct_from(tbl.c.col_b))
        ).fetchall()
        eq_(
            len(result),
            expected_row_count_for_iidt,
