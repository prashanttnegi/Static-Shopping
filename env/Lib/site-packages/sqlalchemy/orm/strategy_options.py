    ...
    def _apply_to_parent(self, parent: Load) -> None:
        ...
        if not orm_util._entity_corresponds_to_use_path_impl(
            ...
            if len(cloned.path) > 1:
                ...
                parent_entity = cloned.path[0]
            _raise_for_doeidt_link(parent.path, attrname, parent_entity)
        ...
        if not orm_util._entity_corresponds_to_use_path_impl(
            ...
            if raiseerr:
                if attr_group and attr is not attr_group[0]:
                    ...
                    _raise_for_doeidt_link(path, str(attr), attr.parent)
            else:
                return None
...
def _raise_for_doeidt_link(path, attrname, parent_entity):
    if len(path) > 1:
